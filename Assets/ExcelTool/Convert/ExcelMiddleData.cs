...
namespace ExcelTool.EditorTool
{
    public class ExcelMiddleData
    {
        ...
        public void Init(DataTable sheet, string excelPath)
        {
            ...
            {
                var prop = sheet.Rows[ExcelConfig.EXCEL_ROW_INDEX_Prop][i].ToString();
                if (string.IsNullOrEmpty(prop)) continue;
                if (prop.StartsWith('#')) continue;
                if (prop.ToLower().Equals("id")) haveIdProp = true;

                var note = sheet.Rows[ExcelConfig.EXCEL_ROW_INDEX_Note][i].ToString();
                var noteArray = note.Split("\n");

                var type = sheet.Rows[ExcelConfig.EXCEL_ROW_INDEX_Type][i].ToString();
                if (string.IsNullOrEmpty(type))
                    throw new Exception("type is null:【" + prop + "】检测到" +
                                        (ExcelConfig.EXCEL_ROW_INDEX_Type + 1) + "行类型为空（可以通知程序加上）, path: " +
                                        excelPath);
            ...
            //convert data
            for (int i = ExcelConfig.EXCEL_ROW_INDEX_Content_Start; i < realRowCount; i++)
            {
                ...
                for (int j = 0; j < realColumns.Count; j++)
                {
                    ...
                    if (propType.Contains("[]"))
                    {
                        ...
                        {
                            Debug.LogError(
                                $"array error,数组错误 title:【{propTitle}】, row:【{i + 1}】, value:【{propValue}】, path:【{excelPath}】");
                        }
                    }
            ...
            var finalString = SplitWithChar(originStr, originType, ';',
                (str) =>
                {
                    return SplitWithChar(str, originType, ',',
                        (str) => { return SplitWithChar(str, originType, '|', null); });
                });
        ...
        private string SplitWithChar(string originStr, string originType, char splitChar,
            Func<string, string> funcSplit)
        {
            StringBuilder sb = new StringBuilder();
