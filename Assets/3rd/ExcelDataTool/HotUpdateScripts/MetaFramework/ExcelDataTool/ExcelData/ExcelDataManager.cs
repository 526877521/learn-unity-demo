...
namespace XFramework.ExcelData
{
    public class ExcelDataManager : SingletonTemplate<ExcelDataManager>
    {
        ...
        public K GetExcelTable<K, V>() where K : EDTableBase<V> where V : EDItemBase
        {
            ...
            var json = AssetDatabase.LoadAssetAtPath<TextAsset>("Assets/3rd/ExcelDataTool/HotUpdateResources/ExcelData/" + jsonName).text;
            if (json == null) return null;
            var asset = JsonConvert.DeserializeObject<K>(json);
